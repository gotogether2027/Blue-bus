...
**Implementation status:** trip stops, trip points, route points, physical inventory, segment allocations, seat holds, bookings, unpaid-booking expiry, customer booking views, unpaid cancellation, and origin/destination search are in the schema. V11 adds provider-neutral payment attempts, a verified-provider-event inbox, refund persistence foundation, and a transactional outbox. **Phase 9.3** adds `RazorpayPaymentProvider` behind `PaymentProvider` (Orders, Checkout HMAC, raw-body webhooks, refunds). **Phase 9.4A** adds immutable `tickets` / `ticket_passengers` snapshots. **Phase 9.4B** processes unpublished `BOOKING_CONFIRMED` outbox rows locally to issue tickets and append `TICKET_ISSUED`. **V22** optionally publishes those outbox rows to RabbitMQ (`rabbit_published_at`) with an idempotent `processed_events` consumer. Confirmed-booking cancellation policy and channel notifications remain separate work.
...
**Opt-in RabbitMQ path (Phase 1):** `BLUE_BUS_RABBITMQ_ENABLED=true` plus broker host. Topology is a durable topic exchange `blue-bus.events`, durable queue `blue-bus.booking-confirmed`, routing key `booking.confirmed`. The UTF-8 JSON envelope preserves `eventId` (the outbox UUID), `eventType`, `aggregateType`, `aggregateId`, `schemaVersion`, `correlationId`, `causationId`, `occurredAt`, and `payload`. Message ID is the outbox event ID. When `consumer-enabled=true`, the consumer issues tickets through the existing `TicketApplicationService` and records `(event_id, consumer_name)` in `processed_events`; the local processor skips `BOOKING_CONFIRMED` so the two paths do not race. Publisher-only (`consumer-enabled=false`) is shadow mode: the local processor still issues tickets.
