...
**Implementation status:** trip stops, trip points, route points, physical inventory, segment allocations, seat holds, bookings, unpaid-booking expiry, customer booking views, unpaid cancellation, and origin/destination search are in the schema. V11 adds provider-neutral payment attempts, a verified-provider-event inbox, refund persistence foundation, and a transactional outbox. **Phase 9.3** adds `RazorpayPaymentProvider` behind `PaymentProvider` (Orders, Checkout HMAC, raw-body webhooks, refunds). **Phase 9.4A** adds immutable `tickets` / `ticket_passengers` snapshots. **Phase 9.4B** processes unpublished `BOOKING_CONFIRMED` outbox rows locally to issue tickets and append `TICKET_ISSUED`. **V22** optionally publishes those outbox rows to RabbitMQ (`rabbit_published_at`) with an idempotent `processed_events` consumer. **V23** adds a channel-agnostic notification foundation (`notifications`, `notification_preferences`) that consumes the same outbox events. Confirmed-booking cancellation policy is implemented; real email/SMS/WhatsApp provider HTTP is not.
...
**Phase 9.4A:** `TicketApplicationService.issueForBooking` is idempotent (`UNIQUE(tickets.booking_id)`). Customer `POST /api/v1/bookings/{bookingId}/tickets`, `GET /api/v1/bookings/{bookingId}/ticket`, and `GET /api/v1/tickets/{ticketId}` use JWT subject ownership (cross-customer `404`). The booking-scoped GET is read-only and does not issue a ticket. **Phase 9.4B:** booking confirmation persists `BOOKING_CONFIRMED` in the same TX; a scheduled local outbox processor (`FOR UPDATE SKIP LOCKED`) issues the ticket and writes `TICKET_ISSUED` atomically, then marks the confirmation event `published_at`. That timestamp is local-handler completion only.

### Outbox pattern for events

**What:** a domain change and an `outbox_event` record commit in the same PostgreSQL transaction. RabbitMQ is never called inside the booking/payment transaction. A later publisher claims unpublished broker rows (`FOR UPDATE SKIP LOCKED`), publishes a durable envelope, waits for publisher confirm, and only then sets `rabbit_published_at`. Delivery is **at-least-once**; consumers must be idempotent. This is not exactly-once delivery.

**Default local path:** `blue-bus.rabbitmq.enabled=false`. The local processor remains authoritative for `BOOKING_CONFIRMED` → ticket.

**Opt-in RabbitMQ path (Phase 1):** `BLUE_BUS_RABBITMQ_ENABLED=true` plus broker host. Topology is a durable topic exchange `blue-bus.events`, durable queue `blue-bus.booking-confirmed` bound to `booking.confirmed`, and durable queue `blue-bus.notifications` bound to `booking.*` / `ticket.*` / `refund.*` / `payment.*`. The UTF-8 JSON envelope preserves `eventId` (the outbox UUID), `eventType`, `aggregateType`, `aggregateId`, `schemaVersion`, `correlationId`, `causationId`, `occurredAt`, and `payload`. Message ID is the outbox event ID. When `consumer-enabled=true`, the ticket consumer issues tickets through the existing `TicketApplicationService` and records `(event_id, consumer_name)` in `processed_events`; the local ticket processor skips `BOOKING_CONFIRMED` so the two paths do not race. When `notification-consumer-enabled=true` together with RabbitMQ, the notification consumer creates notification rows and the local notification processor skips those events. RabbitMQ enabled with `notification-consumer-enabled=false` leaves the local notification processor authoritative; the publisher may still run in shadow mode. Ticket `consumer-enabled=false` is the same shadow pattern for tickets. The publisher now emits every notification-source outbox type (`BOOKING_CONFIRMED`, `TICKET_ISSUED`, `BOOKING_CANCELLED`, `REFUND_REQUESTED`, `REFUND_SUCCEEDED`, `REFUND_FAILED`, `PAYMENT_FAILED`) using per-type routing keys.

**Notification foundation (V23):** `NotificationApplicationService` (consumer `notification-dispatcher`) creates per-enabled-channel rows from those outbox types. `TRIP_CANCELLED` is derived from `BOOKING_CANCELLED` only when the persisted policy is `TRIP_CANCELLED_FULL_REFUND_V1` or `TRIP_CANCELLED_UNPAID_V1`. A customer cancellation reason is not a signal. `REFUND_REQUESTED` is one outbox row per refund. `REFUND_FAILED` is written only when that refund is terminal (`FAILED` and no longer retryable). V24 unique partial indexes enforce one of each per refund aggregate. Default customer preferences enable EMAIL only. Delivery uses `NotificationProvider` logging adapters (local reference `local-{id}`, no external HTTP) with claim/backoff/`FAILED` after `max-attempts`. Idempotency is `processed_events` plus unique `(user, event type, channel, source_event_id)` and `(user, event type, channel, logical_key)`. Customer reads: `GET /api/v1/notifications` and `GET /api/v1/notification-preferences`.

**Failure/retry:** broker down leaves `rabbit_published_at` null and schedules `rabbit_next_retry_at`. Crash after broker ACK but before the DB mark can republish; the consumer treats the same `eventId` as a duplicate. Malformed envelopes are nacked without requeue and create no tickets. Transient consumer failures are requeued and never acked.
...
The highest-risk race is two customers selecting the same seat **over overlapping route segments**. The required defense is a unique per-trip inventory row plus atomic insertion/transition of an active segment allocation protected by a PostgreSQL exclusion constraint, with short transactions and conflict handling. Holds carry an expiry and are reclaimed safely. Unpaid `PENDING_PAYMENT` bookings carry a persisted `payment_expires_at` and are reclaimed by a scheduled reaper (`FOR UPDATE SKIP LOCKED` per booking): booking → `EXPIRED`, items → `EXPIRED`, allocations `BOOKED` → `RELEASED`. A concurrent confirm/cancel that locks the booking first wins entirely; a confirmed booking cannot subsequently expire. Payment failure/expiry cannot leave seats stuck. A late successful payment after a released/expired unpaid booking must never re-book the seat automatically: record it as `SUCCEEDED / REQUIRES_RESOLUTION`, persist one captured-amount compensation refund (`late-payment-{paymentAttemptId}`) in the same payment transaction, and let the V19 worker refund the provider after commit. The booking stays `EXPIRED`/`CANCELLED`; no ticket, hold, or inventory is restored. The payment webhook ledger and payment/provider ID uniqueness make duplicates harmless. The design still needs a load test for hot trips, delayed webhooks, worker crashes between database commit and publish, cancellation while payment is pending, and same-seat overlapping/non-overlapping segment requests.

Payment processing follows the global lock order `booking → payment_attempt → allocations`. Verified success, payment disposition, booking confirmation, provider-event completion, and outbox records commit atomically. Expiry/cancellation winning first leaves the booking terminal and records later captured money as `SUCCEEDED / REQUIRES_RESOLUTION`; it never recreates an allocation. The same payment transaction inserts a durable compensation `refunds.REQUESTED` row when the resolution reason is late/non-travel-valid (`BOOKING_EXPIRED`, `BOOKING_CANCELLED`, `BOOKING_NOT_PAYABLE`, `PAYMENT_AFTER_DEADLINE`). Provider HTTP is not made under those locks; the existing V19 refund worker claims due rows (`FOR UPDATE SKIP LOCKED`) for both `REFUND_PENDING` cancellation refunds and `REQUIRES_RESOLUTION` compensation refunds. Duplicate verified success events reuse the unique `(payment_attempt_id, idempotency_key)` refund. Provider calls occur outside database transactions. Operator/admin trip cancellation locks `trip → bookings (id order) → per-booking allocations/ticket/succeeded payment → active holds (id order)` and never calls Razorpay while those locks are held. After a successful trip-cancel commit, no `CONFIRMED` booking remains on that `CANCELLED` trip. Abandoned `INITIATING` attempts are recovered by a V21 scheduled worker that claims `payment_attempts` only (`FOR UPDATE SKIP LOCKED`, `next_retry_at` lease), calls Razorpay Orders with `X-Razorpay-Idempotency-Key` = `payment_attempt_id` after commit, then applies `INITIATING → PENDING` with the existing booking→payment lock order. The worker never confirms a booking; webhooks remain the confirmation authority. This phase does not poll GET-order.
