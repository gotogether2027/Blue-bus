...
Implemented so far: `GET /api/v1/health`, customer registration + login + refresh/logout + `/auth/me`, admin master-data/trip APIs, **public ACTIVE location discovery** (`GET /api/v1/locations`), public journey seat availability, public temporary seat holds (optional JWT ownership), authenticated hold-to-booking (`/api/v1/bookings`) with payment/ticket/refund summary fields, nested owner reads (`/bookings/{id}/payments`, `/ticket`, `/refunds`), Razorpay payment APIs, **Phase 9.4A tickets** (`POST /api/v1/bookings/{bookingId}/tickets`, `GET /api/v1/bookings/{bookingId}/ticket`, `GET /api/v1/tickets/{ticketId}`), and **V23 notification reads** (`GET /api/v1/notifications`, `GET /api/v1/notification-preferences`). Browser CORS is an explicit `blue-bus.cors.allowed-origins` allow-list (empty by default). Email verification, profile editing, and real provider email/SMS/WhatsApp remain deferred.
...
**Transaction:** lock hold `FOR UPDATE` → validate owned + ACTIVE + unexpired → **re-check trip saleability now** (`SCHEDULED`/`ON_SALE` and booking window open, before departure) → create `PENDING_PAYMENT` booking + passengers + items → mark allocations `HELD`→`BOOKED` → consume hold. If the trip is no longer saleable, the request returns `409`, the hold stays `ACTIVE` (existing expiry reaper still applies), and no booking/allocation conversion occurs. All-or-nothing; GiST exclusion unchanged. Unique-constraint races abort that insert TX (rollback-only); the facade then reads the winner in a fresh transaction and returns the same booking — never continues the failed session. Existing same-key idempotent replays still return the original booking without re-checking saleability.
...
Only `SCHEDULED` / `ON_SALE` trips are returned, and only when the origin trip-stop sequence is strictly less than the destination trip-stop sequence **and the trip is saleable now**: `booking_opens_at <= now`, `booking_closes_at > now`, and `scheduled_departure_at > now`. Saleability is derived at request time from those persisted timestamps plus status; it is not a stored lifecycle state (`ON_SALE`/`CLOSED`/`DEPARTED`/`COMPLETED` are not transitioned by a scheduler). DRAFT, CANCELLED, not-yet-open, closed, and departed trips are omitted. The response includes operator/bus/route identity, base fare, trip-specific boarding points at origin, drop points at destination, and `availableSeatCount` from the existing segment-overlap availability projection (active `HELD`/`BOOKED`/`BLOCKED` allocations on `[origin,destination)`). Adjacent non-overlapping segments remain independently countable. Same origin and destination → `400`. Unknown location → `404`. Empty result is `200 []`.
...
If Razorpay accepts an order and BLUE BUS crashes before persisting `provider_order_id`, the attempt remains `INITIATING` with a null provider order id. Recovery **never confirms a payment or booking**. Verified webhook/checkout remains the confirmation authority.

Two recovery paths share the same lease/backoff worker and provider adapter:
...
Before create-order is retried, the adapter performs Razorpay **GET `/v1/orders?receipt={receipt}`**. Receipt, `notes.payment_attempt_id`, `notes.merchant_reference`, amount (paise), and currency must all match the stored Blue Bus attempt. A unique match persists `provider_order_id` and moves `INITIATING → PENDING` through the existing initiation worker (`PAYMENT_INITIATED` only on that transition). GET not-found leaves the attempt `INITIATING` so the existing create-order retry (`X-Razorpay-Idempotency-Key` = `payment_attempt_id`) can proceed. A returned order that does not match is not associated and create-order is not retried on that pass. Timeout / 5xx / transport failure keep `INITIATING` with bounded exponential backoff (5s … 15 minutes) and invent no provider ids. GET-order is order-id recovery only; it never treats a captured order as payment success.
...
Success processing locks `booking → payment_attempt → allocations`. On-time success for `PENDING_PAYMENT` atomically records `SUCCEEDED / APPLIED_TO_BOOKING` and confirms Booking. If expiry/cancellation won, the payment is `SUCCEEDED / REQUIRES_RESOLUTION`; Booking and released/cancelled allocations stay unchanged; the webhook/checkout response is not a booking confirmation (`processingStatus` `LATE_PAYMENT_COMPENSATION`). The payment transaction inserts one full captured-amount `refunds` row (`late-payment-{paymentAttemptId}`) before returning; the V19 worker performs provider refund HTTP after commit. Duplicate Razorpay events reuse that refund. Amount/currency/reference mismatch follows the same reconciliation path without auto-refund. `GET /api/v1/payments/{id}` continues to report `status=SUCCEEDED` and `disposition=REQUIRES_RESOLUTION` even after the compensation refund becomes `SUCCEEDED`: the disposition records that capture was not applied to the booking, not that money is still held. There is no customer compensation API; public refund remains gated to `REFUND_PENDING`/`REFUNDED`.

Deferred: additional payment providers. Channel notifications have a V23 foundation (logging providers only; no external email/SMS/WhatsApp HTTP).

**Phase 9.4B:** When a booking transitions `PENDING_PAYMENT → CONFIRMED`, the confirmation transaction writes `BOOKING_CONFIRMED` to `outbox_events`. A scheduled local processor (`blue-bus.outbox.processor.*`, default every 5s, batch 50, `FOR UPDATE SKIP LOCKED`) issues the ticket idempotently and writes `TICKET_ISSUED` in the same transaction before marking the confirmation event `published_at`. That field is local-handler completion only. **V22 / optional RabbitMQ:** `blue-bus.rabbitmq.enabled` (default false) publishes outbox rows to durable topic exchange `blue-bus.events`. The ticket queue remains `blue-bus.booking-confirmed` / `booking.confirmed`. The notification queue is `blue-bus.notifications`, bound to `booking.*` / `ticket.*` / `refund.*` / `payment.*`. Publisher confirm still precedes `rabbit_published_at`. Delivery is at-least-once. Ticket consumption is idempotent via `processed_events` plus `uq_tickets_booking`. Notification consumption is idempotent via `processed_events` plus notification unique indexes. When each consumer is enabled, it is authoritative for its work and the matching local processor skips. Local development keeps the local processors. If the booking is no longer `CONFIRMED` because trip cancellation already cascaded it (`CANCELLED` / `REFUND_PENDING` / `REFUNDED` / `EXPIRED`), the ticket handler does not throw `409`, does not create a ticket, and completes the event so it cannot retry forever. Premature `BOOKING_CONFIRMED` while still `PENDING_PAYMENT` remains retryable. Manual ticket POST remains safe.

## Customer notifications — V23 foundation

Channel-agnostic notification records created from existing outbox events. No customer write APIs. Delivery uses local logging providers (`local-{notificationId}`); real provider HTTP is not implemented.

| Method | Path | Auth | Success |
|---|---|---|---|
| `GET` | `/api/v1/notifications` | Bearer JWT (owner) | `200 OK` |
| `GET` | `/api/v1/notification-preferences` | Bearer JWT (owner) | `200 OK` |

`GET /notifications` returns only the authenticated customer's rows (`page` default 0, `size` default 20, max 50), newest first: `id`, `eventType`, `channel`, `templateCode`, `subject`, `body`, `status`, `createdAt`, `sentAt`. Unauthenticated → `401`. Another customer's notifications are never included.

`GET /notification-preferences` returns effective channel flags. Missing rows default to email enabled, SMS/WhatsApp disabled. There is no preference mutation API yet.

Supported notification types: `BOOKING_CONFIRMED`, `TICKET_ISSUED`, `BOOKING_CANCELLED`, `REFUND_REQUESTED`, `REFUND_SUCCEEDED`, `REFUND_FAILED`, `PAYMENT_FAILED`, and derived `TRIP_CANCELLED`. Template codes match the event type name.

## Customer tickets — Phase 9.4A foundation

Booking = commercial transaction. Ticket = immutable customer-facing travel document issued from a `CONFIRMED` booking. One ticket per booking (`UNIQUE(tickets.booking_id)`). Ticket numbers are customer-facing references: `BB` + 8 unambiguous uppercase alphanumerics (alphabet omits `I`/`O`/`0`/`1`), generated with `SecureRandom`, uniqueness enforced by the database. Snapshot fields (operator display name, OD stop names, schedule, passenger/seat/fare) are copied at issuance and do not silently follow later operational edits. PDF/QR remain deferred. Notification rows for `TICKET_ISSUED` are created by the V23 notification consumer from the existing outbox event.
...
Errors follow the existing `ApiError` envelope: `400` validation / bad segment / duplicate seats / blocked inventory / wrong-trip inventory; `404` unknown trip/stop/hold/inventory; `409` seat overlap, non-cancellable status, or a **known trip that is not saleable now** (booking not open, booking closed, trip departed, or trip not on sale). Holds cannot be created outside the saleable window. Idempotent replay of an existing owned hold is unchanged.
...
`availability` is `AVAILABLE` or `UNAVAILABLE`. Validation: missing/malformed params → `400`; unknown trip or stop not on trip → `404`; origin not before destination → `400`; **known trip that is not saleable now → `409`** (same derived saleability rule as search/holds/booking). The segment-overlap algorithm and physical `BLOCKED` behavior are unchanged. No mutation. No authentication in this phase.
...
- `POST /api/v1/admin/trips/{id}/deactivate` — maps to `cancel()`. Cascades passenger handling in the same transaction as the trip row `FOR UPDATE` (same rules as operator cancel). Already-`CANCELLED` remains idempotent. `DEPARTED`/`COMPLETED` remain rejected. No new public endpoint.
...
| `POST` | `/api/v1/operator/{operatorId}/trips/{tripId}/cancel` | `OPERATOR_ADMIN` | `200` → `CANCELLED` (idempotent if already cancelled). Cascades passenger handling in the same DB transaction (see trip-cancel cascade above). Refused from `DEPARTED`/`COMPLETED`. `CONFIRMED` without a captured payment → `409` and full rollback. No new public endpoint. |
...
**Trip administration (Phase 9.5C + trip-cancel cascade):** Dedicated `OperatorTripAdminService` (does not call `TripAdminService`). Create: early `requireMember(OPERATOR_ADMIN)` → lock `buses` then `routes` by `(id, operatorId)` `FOR UPDATE` → revalidate ACTIVE admin + operator → require bus/route ACTIVE → create `DRAFT` with layout from `bus.getSeatLayout()` → exact uniqueness `(bus, serviceDate, departure)` plus same-bus interval overlap check among non-`CANCELLED` trips under the bus lock (`newDeparture < existingArrival AND newArrival > existingDeparture`; adjacent endpoints allowed) → snapshot stops/points/inventory. Existing-trip mutations: trip row `FOR UPDATE` by `(id, operatorId)` → revalidate → commercial PATCH / `schedule()` / `cancel()`. Cancel (same cascade as admin deactivate): authorize → lock trip → lock bookings for `trip_id` ordered by id → per affected booking reuse customer-cancel inventory/ticket/payment writes → cancel ACTIVE holds ordered by id → `trip.cancel()` → commit. No provider HTTP inside that transaction. `CONFIRMED` → `REFUND_PENDING` + `TRIP_CANCELLED_FULL_REFUND_V1` + `refunds.REQUESTED` for the captured amount (after-commit / V19 retry complete `REFUNDED`). `PENDING_PAYMENT` → `CANCELLED` + `TRIP_CANCELLED_UNPAID_V1` (no refund). `EXPIRED`/`CANCELLED`/`REFUNDED` no-op. `REFUND_PENDING` left unchanged (no second cancellation/refund). Missing ticket is valid. `CONFIRMED` without captured payment → `409` and full rollback. `booking_cancellations.requested_by_user_id` is the operator/admin actor. After scheduled departure is allowed when `Trip.cancel()` permits it; `DEPARTED`/`COMPLETED` stay rejected. No new public endpoints. No `ON_SALE`/`CLOSED`/`DEPARTED`/`COMPLETED` transitions. No trip events. No migration for overlap (application check under bus lock).
