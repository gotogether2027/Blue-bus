...
**Phase 9.4B:** When a booking transitions `PENDING_PAYMENT → CONFIRMED`, the confirmation transaction writes `BOOKING_CONFIRMED` to `outbox_events`. A scheduled local processor (`blue-bus.outbox.processor.*`, default every 5s, batch 50, `FOR UPDATE SKIP LOCKED`) issues the ticket idempotently and writes `TICKET_ISSUED` in the same transaction before marking the confirmation event published. If the booking is no longer `CONFIRMED` because trip cancellation already cascaded it (`CANCELLED` / `REFUND_PENDING` / `REFUNDED` / `EXPIRED`), the handler does not throw `409`, does not create a ticket, and marks the event published so it cannot retry forever. Premature `BOOKING_CONFIRMED` while still `PENDING_PAYMENT` remains retryable. Manual ticket POST remains safe.
