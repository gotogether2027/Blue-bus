...
Deferred: additional providers, channel notifications.

**Phase 9.4B:** When a booking transitions `PENDING_PAYMENT → CONFIRMED`, the confirmation transaction writes `BOOKING_CONFIRMED` to `outbox_events`. A scheduled local processor (`blue-bus.outbox.processor.*`, default every 5s, batch 50, `FOR UPDATE SKIP LOCKED`) issues the ticket idempotently and writes `TICKET_ISSUED` in the same transaction before marking the confirmation event `published_at`. That field is local-handler completion only. **V22 / optional RabbitMQ:** `blue-bus.rabbitmq.enabled` (default false) publishes the same outbox row to durable topic exchange `blue-bus.events` / queue `blue-bus.booking-confirmed` / routing key `booking.confirmed` after commit, waiting for publisher confirm before `rabbit_published_at`. Delivery is at-least-once. The consumer is idempotent via `processed_events` plus `uq_tickets_booking`. When the consumer is enabled, it is authoritative for `BOOKING_CONFIRMED` tickets and the local processor skips that type. Local development keeps the local processor. If the booking is no longer `CONFIRMED` because trip cancellation already cascaded it (`CANCELLED` / `REFUND_PENDING` / `REFUNDED` / `EXPIRED`), the handler does not throw `409`, does not create a ticket, and completes the event so it cannot retry forever. Premature `BOOKING_CONFIRMED` while still `PENDING_PAYMENT` remains retryable. Manual ticket POST remains safe.
