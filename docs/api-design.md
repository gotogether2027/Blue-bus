...
- `POST /api/v1/admin/trips/{id}/deactivate` — maps to `cancel()`. Blocked with `409` when any booking on the trip is `CONFIRMED` or `REFUND_PENDING` (trip row `FOR UPDATE` first). `PENDING_PAYMENT` / `EXPIRED` / `CANCELLED` / `REFUNDED` do not block. Already-`CANCELLED` remains idempotent. **No cascade:** does not cancel bookings, tickets, allocations, or refunds.
...
| `POST` | `/api/v1/operator/{operatorId}/trips/{tripId}/cancel` | `OPERATOR_ADMIN` | `200` → `CANCELLED` (idempotent if already cancelled). Status-only: does not cancel bookings, refunds, tickets, holds, inventory, or write outbox. Refused from `DEPARTED`/`COMPLETED`. **Blocked with `409`** while any booking is `CONFIRMED` or `REFUND_PENDING` (checked while the trip row `FOR UPDATE` lock is held). Unpaid `PENDING_PAYMENT` and terminal `EXPIRED`/`CANCELLED`/`REFUNDED` do not block. No trip-cancellation cascade/refund exists yet. |
