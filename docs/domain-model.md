...
- **Trip:** one scheduled journey of one bus on one route, with an operating/service date. Operator administration (Phase 9.5C) creates trips as `DRAFT`, schedules to `SCHEDULED`, and cancels to `CANCELLED` (status-only; no booking/refund/ticket cascade). Commercial terms may change while `DRAFT`/`SCHEDULED`. Bus/route/layout/schedule are immutable after create. Snapshots (`trip_stops`, `trip_points`, `trip_seat_inventory`) are written at create and never rewritten by master edits. `ON_SALE`/`CLOSED`/`DEPARTED`/`COMPLETED` transitions are not implemented; those enum values are not persisted by a scheduler. **Saleable now** is derived at request time: status `SCHEDULED` or `ON_SALE`, `booking_opens_at <= now`, `now < booking_closes_at`, and `now < scheduled_departure_at`. Customer search/availability/holds/booking conversion use that predicate. Operator/admin trip cancel is blocked while any booking is `CONFIRMED` or `REFUND_PENDING`; unpaid and already-terminal bookings do not block. There is no trip-cancellation cascade refund yet.
