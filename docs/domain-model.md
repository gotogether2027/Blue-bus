...
- **Seat layout:** reusable template describing physical seats; a layout version must not be silently changed after trips use it. Operator Phase 9.5F creates `DRAFT` layouts with ≥1 seat, allows DRAFT-only metadata PATCH, publishes to `PUBLISHED`, and archives to `ARCHIVED`. Seats are immutable after create. Archiving does not detach buses or rewrite `trip_seat_inventory`.
...
- **Trip:** one scheduled journey of one bus on one route, with an operating/service date. Operator administration (Phase 9.5C) creates trips as `DRAFT`, schedules to `SCHEDULED`, and cancels to `CANCELLED`. Commercial terms may change while `DRAFT`/`SCHEDULED`. Bus/route/layout/schedule are immutable after create. Snapshots (`trip_stops`, `trip_points`, `trip_seat_inventory`) are written at create and never rewritten by master edits. `ON_SALE`/`CLOSED`/`DEPARTED`/`COMPLETED` transitions are not implemented; those enum values are not persisted by a scheduler. **Saleable now** is derived at request time: status `SCHEDULED` or `ON_SALE`, `booking_opens_at <= now`, `now < booking_closes_at`, and `now < scheduled_departure_at`. Customer search/availability/holds/booking conversion use that predicate. Operator/admin trip cancel (including after scheduled departure when `Trip.cancel()` permits it) cascades passenger handling in the same DB transaction: `CONFIRMED` → `REFUND_PENDING` with a full captured refund (`TRIP_CANCELLED_FULL_REFUND_V1`); `PENDING_PAYMENT` → `CANCELLED` unpaid (`TRIP_CANCELLED_UNPAID_V1`); `EXPIRED`/`CANCELLED`/`REFUNDED` no-op; existing `REFUND_PENDING` left unchanged (no second cancellation/refund). Active `HELD` seat holds are cancelled. `CONFIRMED` without a captured `SUCCEEDED` payment fails the entire cancel with `409` and rolls back. `DEPARTED`/`COMPLETED` remain rejected. Late payment after unpaid trip-cancel stays `SUCCEEDED` / `REQUIRES_RESOLUTION` and is refunded as late-payment compensation (`late-payment-{paymentAttemptId}`), never as a trip-cancellation refund.
- **Trip inventory:** the physical-seat snapshot for one trip. It is independent from the bus's permanent seat definition. Physical status is only `AVAILABLE`/`BLOCKED` (global for the trip seat). Operator Phase 9.5D may block/unblock physical status with a reason; this does not create or release segment allocations and does not cancel holds/bookings. Segment-specific blocking is out of scope.
...
Customer-facing journey availability for a requested OD segment is a **read-only projection** (`SeatAvailabilityService`): physical `AVAILABLE`/`BLOCKED` inventory plus overlapping active allocations (`HELD`/`BOOKED`/`BLOCKED`). It is not a stored column. A `HELD` row whose `expires_at` is past still blocks until an explicit expire/cancel transition (no implicit expiry in the projection). The HTTP availability API, hold create, search, and hold-to-book conversion **gate on derived saleability** (status + persisted booking window + departure vs server/`timestamptz` now) before projecting or mutating. The inventory algorithm itself is unchanged.

Customer-facing UI vocabulary `AVAILABLE` / `HELD` / `BOOKED` / `BLOCKED` for a requested journey remains **derived** from physical inventory plus overlapping active allocations. It is not stored as a whole-trip flag on `TripSeatInventory`. Physical inventory status is only `AVAILABLE` or `BLOCKED`. After an operator physical block, new holds fail the existing `AVAILABLE` check; existing `HELD` allocations remain convertible to `BOOKED`, and existing `BOOKED` allocations remain valid. GiST segment exclusion is unchanged.
...
| BookingCancellation (V12/V18/V20) | COMPLETED (customer unpaid/confirmed, or trip-cancelled unpaid/full-refund) |
...
| Notification (V23) | PENDING, SENT, FAILED |
| Notification channel (V23) | EMAIL, SMS, WHATSAPP |
...
V21 recovery moves stale `INITIATING` attempts with a null `provider_order_id` to `PENDING`. The worker first reconciles via Razorpay GET-order using the existing create-order receipt and notes (`payment_attempt_id`, `merchant_reference`, amount, currency). A unique match persists the provider order id only. GET not-found allows the existing create-order retry (`payment_attempt_id` idempotency key). Timeout/5xx stay `INITIATING` with lease + bounded backoff and no hard attempt cap. Recovery never confirms a booking or issues a ticket; `PAYMENT_INITIATED` is emitted only on a real `INITIATING → PENDING` transition. If the booking is no longer payable when initiation completes, existing expiry semantics apply (`EXPIRED` with the provider order stored). Verified webhook/checkout confirmation remains authoritative.

`REQUIRES_RESOLUTION` means the payment was successfully captured but could not be applied to the booking and required compensation/reconciliation; successful compensation does not change the original payment disposition. The booking is not confirmed, no ticket is issued, and allocations/inventory are not restored. For late success after `EXPIRED`/`CANCELLED`/non-payable/post-deadline, the payment transaction also inserts one `refunds` row (`REQUESTED`, amount = persisted captured amount, idempotency `late-payment-{paymentAttemptId}`, reason `LATE_PAYMENT_COMPENSATION`). The existing V19 refund worker then calls the provider outside booking locks (`refund.id` as Razorpay idempotency key) until the **refund** is `SUCCEEDED`. Duplicate webhooks reuse that single refund. Amount/currency/reference mismatch and extra success against an already-confirmed booking stay `REQUIRES_RESOLUTION` without auto-refund. Compensation refund success does not move the booking to `CONFIRMED` or `REFUNDED` and does not move payment disposition off `REQUIRES_RESOLUTION`; `markRefunded` remains a no-op unless the booking is `REFUND_PENDING`. Customer/trip-cancellation refunds stay on `booking-cancel-{cancellationId}` and `CONFIRMED` → `REFUND_PENDING` → `REFUNDED`.
