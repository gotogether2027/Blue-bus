...
TripSeatInventory 1---* Seat allocation (future) *---1 Seat hold / Booking item (future)
...
V4 `trip_seats` (whole-trip `HELD`/`BOOKED`, `locked_until`, `booking_id`) was replaced in V5. Allocation, hold, and booking aggregates are approved but not implemented.
