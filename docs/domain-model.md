...
V21 recovery moves stale `INITIATING` attempts with a null `provider_order_id` to `PENDING` by retrying the existing Razorpay Orders call (`payment_attempt_id` idempotency key, lease + bounded backoff, no hard attempt cap). Uncertain provider failures stay `INITIATING`. Recovery never confirms a booking; `PAYMENT_INITIATED` is emitted only on a real `INITIATING → PENDING` transition. If the booking is no longer payable when initiation completes, existing expiry semantics apply (`EXPIRED` with the provider order stored). Webhook confirmation remains authoritative. GET-order polling is out of scope for this phase.
