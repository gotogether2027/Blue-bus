...
**Implemented in schema/entities:** identity, operators, fleet, locations, routes/stops, route points, trips with `service_date`/`time_zone`, trip-stop and trip-point snapshots, and physical `trip_seat_inventory`. **Phase 8.1–8.2:** email/password login (JWT), customer self-registration with automatic `CUSTOMER` role, and `GET /api/v1/auth/me`. Fine-grained admin RBAC and operator-scoped HTTP authorization remain outstanding.
