...
**Implemented:** segment inventory/allocations, seat holds + reaper, journey availability, auth/refresh, hold-to-book, unpaid-booking expiry, V11 payment foundation, V12 customer booking views/unpaid cancellation/search, **Phase 9.3 Razorpay**, **Phase 9.4A Ticket Foundation**, **Phase 9.4B automatic ticket issuance**, **Phase 9.6 confirmed booking cancellation & refund orchestration** (`CONFIRMED`→`REFUND_PENDING`→`REFUNDED`, full captured refund, allocation/ticket cancel, `refunds.REQUESTED` in the cancel TX, scheduled refund retry with V19 `next_retry_at`/`attempt_count`, orphan public refund gated), **trip saleability enforcement** (request-time `SCHEDULED`/`ON_SALE` + booking window + before departure on search/availability/holds/booking conversion), **trip-cancellation passenger handling** (operator/admin cancel cascades `CONFIRMED`→`REFUND_PENDING` with `TRIP_CANCELLED_FULL_REFUND_V1` and V19 refund retry, `PENDING_PAYMENT`→`CANCELLED` unpaid, skips `REFUND_PENDING`/`EXPIRED`/`CANCELLED`/`REFUNDED`, cancels active holds, trip `CANCELLED` in the same TX, no provider HTTP under locks, V20 policy constraint, `BOOKING_CONFIRMED` outbox skip when the booking is no longer `CONFIRMED`), **late-payment compensation** (`SUCCEEDED` / `REQUIRES_RESOLUTION` after expiry/unpaid cancel never resurrects the booking or issues a ticket; one captured-amount `refunds` row is persisted in the payment TX and completed by the existing V19 worker; cancellation refunds stay on `booking-cancel-*`), and **INITIATING payment recovery** (V21 `attempt_count`/`next_retry_at`, scheduled lease worker retries Razorpay Orders with `payment_attempt_id` idempotency, `INITIATING→PENDING` only, webhook remains confirmation authority, no GET-order polling), and **customer journey completeness** (public `GET /api/v1/locations`, owner nested payment/ticket/refund reads, booking list/detail payment/ticket/refund summaries, fail-closed `blue-bus.cors.allowed-origins`). **Still outstanding:** real email/SMS/WhatsApp provider adapters (V23 has the notification foundation and logging providers only).

## Phase 4 — payments and transactional communication

**Phase 9.3 done:** Razorpay is wired behind the V11 `PaymentProvider` adapter with signed Checkout/webhook rules and refund execution. **Phase 9.4A/B done:** Ticket domain + ownership APIs + local DB outbox processor issuing tickets after confirmation. **RabbitMQ outbox Phase 1 done:** optional `spring-boot-starter-amqp` publisher/consumer for `BOOKING_CONFIRMED` tickets (`rabbit_published_at` + `processed_events`, at-least-once, local processor remains the default). **Notification foundation (V23) done:** channel-agnostic `notifications` / `notification_preferences`, reuse of existing outbox types plus `REFUND_REQUESTED` and derived `TRIP_CANCELLED`, notification queue `blue-bus.notifications`, logging `NotificationProvider` adapters, retry/backoff, and owner-scoped GET APIs. Remaining: real provider HTTP. Preserve V11's database idempotency and booking-first lock order.
