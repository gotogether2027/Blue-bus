...
**Implemented:** segment inventory/allocations, seat holds + reaper, journey availability, auth/refresh, hold-to-book, unpaid-booking expiry, V11 payment foundation, V12 customer booking views/unpaid cancellation/search, **Phase 9.3 Razorpay**, **Phase 9.4A Ticket Foundation**, **Phase 9.4B automatic ticket issuance**, and **Phase 9.6 confirmed booking cancellation & refund orchestration** (`CONFIRMED`→`REFUND_PENDING`→`REFUNDED`, full captured refund, allocation/ticket cancel, `refunds.REQUESTED` in the cancel TX, scheduled refund retry with V19 `next_retry_at`/`attempt_count`, orphan public refund gated). **Still outstanding:** RabbitMQ publishing/consumers, PDF/QR, channel notifications, and operational reconciliation of abandoned `INITIATING` attempts that the customer never retries.
