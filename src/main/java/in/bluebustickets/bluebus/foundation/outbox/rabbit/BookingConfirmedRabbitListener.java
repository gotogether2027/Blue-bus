...
/**
 * Manual-ack BOOKING_CONFIRMED listener. Ack only after the consumer transaction
 * commits. Transient failures are requeued. Malformed envelopes are not.
 */
@Component
@ConditionalOnBean(BookingConfirmedRabbitConsumer.class)
...
public class BookingConfirmedRabbitListener {

    private static final Logger log = LoggerFactory.getLogger(BookingConfirmedRabbitListener.class);

    private final BookingConfirmedRabbitConsumer consumer;
    private final ObjectMapper objectMapper;

    public BookingConfirmedRabbitListener(
            BookingConfirmedRabbitConsumer consumer,
            ObjectMapper objectMapper) {
        this.consumer = consumer;
        this.objectMapper = objectMapper;
    }

    @RabbitListener(queues = "${blue-bus.rabbitmq.queue}", ackMode = "MANUAL")
    public void onMessage(
        ...
        try {
            ...
            log.warn(
                    "RabbitMQ consumer malformed envelope messageId={}: {}",
                    messageId,
                    exception.getMessage());
            nack(channel, deliveryTag, false, messageId);
        } catch (RuntimeException exception) {
            log.warn(
                    "RabbitMQ consumer failure messageId={}: {}",
                    messageId,
                    exception.getMessage());
    ...
    private static void nack(Channel channel, long deliveryTag, boolean requeue, String messageId) {
        try {
            ...
            log.warn(
                    "RabbitMQ consumer nack failed messageId={} requeue={}: {}",
                    messageId,
                    requeue,
